...
export async function scanQrFromPdf(file: File): Promise<string> {
  ...
  for (const pageNum of pagesToScan) {
    ...
    if (result) {
      console.log(
        `[PDF Scanner] Page ${pageNum} result:`,
      ...
      if (!foundInvalidQr) {
        console.warn(
          `[PDF Scanner] Invalid Swiss QR on page ${pageNum}, continuing search...`,
        );
  ...
  if (foundInvalidQr) {
    console.warn(
      "[PDF Scanner] Only invalid QR codes found, returning the first one.",
    );
